...
export const MyIcon = ({ icon, label, hidden, disabled, ...props }) => {
    const IconComponent = iconMap[icon];
    return hidden ? (_jsx(_Fragment, {})) : (_jsxs("div", { className: "flex flex-col items-center", children: [_jsx(IconComponent, { className: `cursor-pointer ${disabled
                    ? "text-gray-400 dark:text-gray-700"
                    : "text-teal-700 dark:text-gray-400"}`, onClick: disabled ? () => { } : props.onClick, ...props }), _jsx("div", { className: "text-xs text-gray-500 font-bold", children: label })] }));
};
